...
class ProjectEventsSubscriber(
    ...
    private fun createProject(id: UUID, title: String, creatorId: String) {
        ...
        pURepositoryProjectUser.save(
            ProjectUsersViewDomain.ProjectUser(
                id = UUID.randomUUID(),
    ...
    private fun createTag(projectId: UUID, tagId: UUID, tagName: String, color: String) {
        ...
        pTSRepositoryProjectStatus.save(
            ProjectTasksStatusesViewDomain.ProjectStatus(
                id = UUID.randomUUID(),
                projectId,
                tagId
            )
    ...
    private fun tagAssignedToTask(projectId: UUID, taskId: UUID, tagId: UUID) {
        ...
        pTRepositoryTask.findById(taskId).get().tag = tagId
    ...
    private fun tagDeleted(projectId: UUID, tagId: UUID) {
        pTSRepositoryStatus.deleteById(tagId)
        pTSRepositoryProjectStatus.deleteByProjectId(projectId)
    }

    private fun userAssignedToTask(projectId: UUID, userId: UUID, taskId: UUID) {
        pTRepositoryTask.findById(taskId).get().userId = userId


    }

    private fun taskNameChanged(projectId: UUID, taskId: UUID, title: String) {
        pTRepositoryTask.findById(taskId).get().name = title
